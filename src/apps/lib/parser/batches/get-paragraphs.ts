function displayCategory(
  ...
  if (node instanceof Element) {
    ...
    if (first === 'flex') return 'block';
    if (first === '-webkit-box') return 'block'; // Old name of flex? Still used on Google Search for some reason.
    if (first === 'grid') return 'block';
    if (first.startsWith('table')) return 'block';
    if (first.startsWith('flow')) return 'block';
    if (first === 'ruby') return 'ruby';
    if (first.startsWith('ruby-text')) return 'ruby-text';
    if (first.startsWith('ruby-base')) return 'inline';
    if (first.startsWith('math')) return 'inline';
    if (display.includes('list-item')) return 'block';
    if (first === 'contents') return 'inline';
    if (first === 'run-in') return 'block';
  }
