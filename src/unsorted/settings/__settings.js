...
import { showError } from '../../unsorted/toast.ts/index.js';
import { assert, nonNull, wrap } from '../../unsorted/util.js';
...
try {
  ...
  const inputFilePicker = nonNull(document.querySelector('#import-file-picker'));
  inputFilePicker.addEventListener('change', async () => {
    try {
      const files = inputFilePicker.files;
      // console.log(files);
      if (files === null || files.length === 0) return;
      const fileContents = await wrap(new FileReader(), (reader, resolve, reject) => {
        reader.onload = () => {
