(async () => {
  ...
  const { displayCategory, Fragment, Paragraph } = await $import(
    "/content/parse.js"
  );
  const { showError } = await $import("/content/toast.js");
  const { parseParagraphs } = await $import("/integrations/common.js");
  ...
  function paragraphsInRange(range) {
    ...
    if (range.startContainer instanceof CharacterData) {
      ...
      while (startNode.nextSibling === null) {
        if (startNode.parentNode === null)
          throw new Error(
            "Selection started at end of node, but there was no next node"
          );
    ...
    if (range.endContainer instanceof CharacterData) {
      ...
      while (endNode.previousSibling === null) {
        if (endNode.parentNode === null)
          throw new Error(
            "Selection ended at start of node, but there was no previous node"
          );
    ...
    const fragments = []; // Fragments that are part of the current paragraphs
    const paragraphs = []; // All paragraphs found so far
