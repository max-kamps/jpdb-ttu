...
export function visibleObserver(enterCallback, exitCallback) {
    const elementVisibleObserver = new IntersectionObserver((entries, _observer) => {
        try {
...
export function parseVisibleObserver(filter = () => true) {
    ...
    const visible = visibleObserver(elements => {
        ...
        for (const element of elements) {
            if (pendingBatches.get(element) !== undefined)
                continue;
...
export function parseParagraphs(paragraphs) {
    ...
    const applied = batches.map(({ paragraph, promise }) => promise
        .then(tokens => {
