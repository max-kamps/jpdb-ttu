...
export function displayCategory(node) {
  if (node instanceof Text || node instanceof CDATASection) {
    ...
    alert(
      `Warning: Unknown display value ${display.join(" ")}, please report this!`
    );
    return "none";
  } else {
    return "none";
...
export function applyTokens(fragments, tokens) {
  ...
  for (const token of tokens) {
    ...
    while (curOffset < token.start) {
      ...
      wrap(
        fragment.node,
        jsxCreateElement("span", { class: "jpdb-word unparsed" })
      );
    ...
    while (curOffset < token.end) {
      ...
      const className = `jpdb-word ${token.card.cardState.join(" ")}`;
      const wrapper =
        ...
        for (const ruby of token.rubies) {
          if (ruby.start >= fragment.start && ruby.end <= fragment.end) {
            ...
            if (ruby.end < fragment.end) {
              ...
              insertAfter(
                jsxCreateElement("rt", { class: "jpdb-furi" }, ruby.text),
                fragment.node
              );
              fragment = fragment = fragments[++fragmentIndex];
            } else {
